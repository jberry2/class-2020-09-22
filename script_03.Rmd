...
- Find course enrollment numbers from Sep 1
- Find course enrollment numbers from today
- we should be careful to select a useful estimand to analyze


After we have a model, Temperance suggests we should:

- Get rid of uncertainty and recognize the limitations of our model
- Focus on internal and external validity
- Internal validity established because of a census
- External validity difficult to establish - can't generalize outside Harvard
...
full_join %>%
  ggplot(mapping = aes(x = reorder(title.x, -difference), y = difference, 
                       fill = ifelse(difference > 0, "red", "dodgerblue"))) +
  geom_col() +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none") +
  labs(
    title = "Changes in Course Enrollment (As of 9/22/2020)",
    x = "Course",
